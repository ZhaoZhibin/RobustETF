...
figure(4);clf;
set(gcf, 'NumberTitle','off','Name','8d_Robust_Experiment');
...
% save figure
SaveFigureLinchao('Bearing_Data',FlagFigureAutoSave,currentFolder)

%% Print the time domain

figure(8);clf;
set(gcf, 'NumberTitle','off','Name','8h_Robust_Experiment');
set(gcf, 'Units', 'centimeters');
set(gcf,'position',WindowPosition);
set(gcf, 'PaperPositionMode', 'auto');   
h1_ap=axes('position',h1); 
hold on
ph(1) = plot(points(1:100), x(1:100), 'b-', 'LineWidth', LineWidth);
ph(2) = plot(points(1:100), x_robust(1:100), 'r-', 'LineWidth', LineWidth);
hold off
legend1 = legend(ph, 'Original', 'RobustETF');
set(legend1,'location','northwest','Orientation','vertical', 'FontSize',FontSize,'FontName',FontName, 'Interpreter', 'latex')
legend boxoff

xlim_min = 0; xlim_max = max(points(1:100));
ylim_min = min(x(1:100));            ylim_max = max(abs(x(1:100)));  
xylim = [xlim_min,xlim_max,ylim_min,ylim_max]; axis(xylim);
...
annotation('textbox',[0 1 0.03 0.03],'String',{'(d)'},'FontSize',FontSize+1,'FontName',FontName,'FontWeight','bold','FitBoxToText','off','LineStyle','none');
% save figure
SaveFigureLinchao('Bearing_Data_Enlargement',FlagFigureAutoSave,currentFolder)
